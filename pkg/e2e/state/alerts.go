...
var _ = ginkgo.Describe(clusterStateTestName, func() {
	...
	ginkgo.It("should have no alerts", func() {
		...
		Expect(err).NotTo(BeNil(), "error running alert gatherer")

		// get results
		results, err = r.RetrieveResults()
		Expect(err).NotTo(BeNil(), "error retrieving results from alert gatherer")

		queryJSON := query{}
		err = json.Unmarshal(results["alerts.json"], &queryJSON)
		Expect(err).NotTo(BeNil(), "error unmarshalling json from alert gatherer")

		clusterProvider, err := providers.ClusterProvider()
		Expect(err).NotTo(BeNil(), "error retrieving cluster provider")
